...
public class PartitionGraph {
    ...
    public void script(String importFileName, String exportFileName) {
        ...
        String webrootPath = "";
        try {
        	webrootPath = new File(HeadlessSimple.class.getResource("/").toURI()).getAbsolutePath();
        	String filePath = webrootPath + "/gephi/"+importFileName+".gexf";
        	
        	/*System.out.println(filePath+"/newf.gexf");*/
        	//这个文件在哪里 哪个？
        ...
        for(Part j:m)
        {
        	System.out.println(j.getDisplayName()+"  "+j.getDisplayName());
        	Object[] el = j.getObjects();
        	for(Object o:el)
        	{
        ...
        //Export
        try {
        	ec.exportFile(new File(exportFileName));
            ec.exportFile(new File(exportFileName+".pdf"));
        } catch (IOException ex) {
            ex.printStackTrace();
            return;
        }
